 ...
 */
function lowerCaseStrings(/* code here */ strings) {
  /* code here */
  let lowerCaseHolder = [];
  strings.forEach(string => lowerCaseHolder.push(string.toLowerCase()));
  if (lowerCaseHolder.length === strings.length) return lowerCaseHolder;
}
 ...
 */
function isItAnApple(/* code here */ strings) {
  /* code here */
  const myArr = [];
  strings.map(string => {
    string.includes("apple") ? myArr.push(true) : myArr.push(false);
  });
  return myArr;
}
 ...
 */
function removeApple(/* code here */ strings) {
  /* code here */
...
function getFullNames(/* CODE HERE */ runners) {
  ...
  const myArr = [];
  runners.map(fulName =>
    myArr.push(`${fulName.last_name}, ${fulName.first_name}`)
  );
  return myArr;
}
//OR
